...
public class Coder {
    public static String coder(String message, char[][] key) {
        int rows = key.length;
...
//case same rows
            if (let1_row == let2_row) {
                //first letter set
                if (let1_col == key[0].length - 1) {
                    msg[i] = key[let1_row][0];
                } else {
                    msg[i] = key[let1_row][let1_col + 1];
                }
                //second letter set
                if (let2_col == key[0].length - 1) {
                    msg[i + 1] = key[let1_row][0];
                } else {
...
//case same columns
                ...
                //first letter set
                if (let1_row == key.length - 1) {
                    msg[i] = key[0][let1_col];
                } else {
                    msg[i] = key[let1_row + 1][let1_col];
                }
                //second letter set
                if (let2_row == key.length - 1) {
                    msg[i + 1] = key[0][let1_col];
                } else {
